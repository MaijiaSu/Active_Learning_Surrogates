function data = PlotHighDimSurfaceV2(Myfun,bound,density,xlabelstr)
...
tt = tiledlayout(Ndim,Ndim,'TileSpacing','tight','Padding','tight')

%% drawing graph at the lower triangle
for ii = 1:NofFig-Ndim
%     subplot(Ndim,Ndim,FigNov(ii))
    ax = nexttile(FigNov(ii),[1,1]);
    fig_id = [FigID(ii,2),FigID(ii,1)];
...
%     shading interp
    ...
    alpha 0.7
    view(0,90)
    if numel(xlabelstr) == Ndim
        xlabel(xlabelstr(fig_id(1)));
        ylabel(xlabelstr(fig_id(2)));
    else
...
% Caxisbound = [min(Ybound(:,1)),max(Ybound(:,2))];
% Caxisbound = [-15,15];
% for ii = 1:NofFig-Ndim
%     ax = nexttile(FigNov(ii),[1,1]);
%     colormap(slanCM('seismic'))
...
% Create a colorbar for one of the plots
% hcb = colorbar(ax);
% set(hcb,'Position',[0.8 0.4 0.04 0.5])
